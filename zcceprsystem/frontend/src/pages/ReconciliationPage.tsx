...
import {
  ...
  useTheme, alpha, List, ListItem, ListItemIcon, ListItemText, MenuItem
} from '@mui/material';
import {
...
<div class="page-footer">
  ...
  return (
    <Box>
                ...
                <Table>
                  ...
                  <TableBody>
                    {pagedMyRequests.map((req) => {
                      const daysElapsed = req.status === 'DISPATCHED' ? calcWorkingDaysFromNow(req.dispatched_at) : null;
                      const isOverdue = daysElapsed !== null && daysElapsed > 5;
                      const daysLeft = daysElapsed !== null ? 5 - daysElapsed : null;
                      return (
                          ...
                          {daysElapsed !== null ? (
                            <Box display="flex" flexDirection="column" gap={0.3}>
                              <Chip
                                ...
                                size="small"
                              />
                              <Typography variant="caption" color="text.secondary">5-day limit</Typography>
                            </Box>
                          ) : (
      ...
      {activeTab === (isLeadOrHOP ? 4 : 2) && isFinanceOrAdmin && (
        <Paper elevation={0} sx={{ border: `1px solid ${theme.palette.divider}` }}>
          {/* Filters */}
          <Box p={2} borderBottom={`1px solid ${theme.palette.divider}`}>
            <Grid container spacing={2} alignItems="center">
      ...
      {activeTab === (isLeadOrHOP ? 5 : 3) && isFinanceOrAdmin && (
        <Paper elevation={0} sx={{ border: `1px solid ${theme.palette.divider}` }}>
          {history.length === 0 ? (
            <Box py={6} textAlign="center">
              <HistoryIcon sx={{ fontSize: 64, color: 'text.disabled', mb: 2 }} />
      ...
      {/* ==================== RECONCILIATION FORM DIALOG ==================== */}
      <Dialog open={formOpen} onClose={() => { setFormOpen(false); resetFormState(); }} maxWidth="md" fullWidth>
        <DialogTitle>
        ...
        <DialogContent dividers>
          {selectedRequest && (
            <Box>
              {(() => {
                const daysElapsed = calcWorkingDaysFromNow((selectedRequest as any).dispatched_at);
                if (daysElapsed === null) return null;
                const isLate = daysElapsed > 5;
                ...
                return (
                  ...
                  >
                    {isLate
                      ? <><strong>Late Submission:</strong> You are submitting {daysElapsed} working day{daysElapsed !== 1 ? 's' : ''} after dispatch. The 5-day deadline has passed — this will be marked as a late reconciliation.</>
                      : daysLeft === 0
                        ? <><strong>Due Today:</strong> You have used all 5 working days. Submit now to remain on time.</>
                        : <><strong>On Time:</strong> {daysElapsed} working day{daysElapsed !== 1 ? 's' : ''} since dispatch — {daysLeft} day{daysLeft !== 1 ? 's' : ''} remaining within the 5-day deadline.</>
                    }
                  </Alert>
                );
              })()}
