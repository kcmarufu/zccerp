...
exports.getAllProjects = async (req, res) => {
  try {
    ...
    // Non-Finance HOPs/LEADs: scope to projects in their department
    // (either p.department_id matches or a budget line for the project does)
    if ([ROLES.HEAD_OF_PROGRAMS, ROLES.PROGRAM_LEAD].includes(req.user.role) && !isFinanceManager(req.user)) {
      where += ' AND (p.department_id = ? OR EXISTS (SELECT 1 FROM budget_lines bl WHERE bl.project_id = p.id AND bl.department_id = ?))';
    ...
    const projects = await query(
      `SELECT p.*,
              ...
              COALESCE(
                dep.department_name,
                (SELECT dep2.department_name FROM departments dep2
                 JOIN budget_lines bl2 ON bl2.department_id = dep2.id
                 WHERE bl2.project_id = p.id LIMIT 1)
              ) AS department_name,
              COALESCE(
                dep.department_code,
                (SELECT dep2.department_code FROM departments dep2
                 JOIN budget_lines bl2 ON bl2.department_id = dep2.id
                 WHERE bl2.project_id = p.id LIMIT 1)
              ) AS department_code,
