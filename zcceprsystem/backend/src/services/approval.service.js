...
class ApprovalService {
  ...
  async getPendingApprovals(role, userId, departmentId, filters = {}) {
    ...
    switch (role) {
      case ROLES.PROGRAM_LEAD:
        // Lead sees:
        //   - Own-dept requests that have NO cross-dept routing (r.routing_department_id IS NULL)
        //   - Cross-dept requests explicitly routed TO their dept (r.routing_department_id = this dept)
        //   - ALL admin-donor requests (any Lead can approve those)
      ...
      case ROLES.HEAD_OF_PROGRAMS:
        // HOP oversees all departments — sees all pending approvals
  ...
  async getApproverStats(role, userId, departmentId) {
    ...
    switch (role) {
      case ROLES.PROGRAM_LEAD:
        // Lead sees own-dept requests + cross-dept requests routed to their dept + ALL admin-donor requests
        ...
        break;
      case ROLES.HEAD_OF_PROGRAMS:
        // HOP oversees all departments — sees all pending approvals including Admin-donor requests
