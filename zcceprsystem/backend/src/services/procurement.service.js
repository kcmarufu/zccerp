...
class ProcurementService {
  ...
  async getPurchaseRequests(user, filters = {}) {
    ...
    if (user.role === ROLES.GENERAL_USER) {
      ...
      params.push(user.id);
    } else if (user.role === ROLES.PROGRAM_LEAD) {
      // Program lead sees requests in their department OR cross-dept requests routed to their dept
      // plus any they have acted on
      where = `WHERE (
        pr.department_id = ? OR
        (pr.status = 'PENDING_DEPT_APPROVAL' AND (pr.routing_department_id = ? OR pr.routing_department_id IS NULL)) OR
    ...
    } else if (user.role === ROLES.HEAD_OF_PROGRAMS) {
      // Head of Programs oversees all departments — sees everything
      where = 'WHERE 1=1';
    } else {
  ...
  async committeeDecision(requestId, decision, selectedQuotationId, user, justification = '') {
    ...
    // Verify the committee member has a designated seat
    const userDetails = await query('SELECT committee_seat FROM users WHERE id = ?', [user.id]);
    const seat = userDetails[0]?.committee_seat;
    if (!seat && user.role !== ROLES.ADMIN) {
      throw new Error('Your account has no committee seat assigned. Contact the system administrator.');
    }

    const REQUIRED_SEATS = ['HSD', 'CPJS', 'FINANCE_ADMIN'];

    return transaction(async (conn) => {
      if (seat) {
        // UPSERT: insert vote or update existing vote for this seat
        // This allows a member to change a previous REJECTED vote to APPROVED
        const [existingVote] = await conn.execute(
          'SELECT id FROM proc_committee_votes WHERE request_id = ? AND committee_seat = ?',
          [requestId, seat]
        );
        ...
        if (existingVote.length > 0) {
          ...
          await conn.execute(
            `UPDATE proc_committee_votes SET vote = ?, justification = ?, voter_id = ?, voted_at = NOW()
             WHERE request_id = ? AND committee_seat = ?`,
            [decision, justification || null, user.id, requestId, seat]
          );
        } else {
          // Insert new vote
          await conn.execute(
  ...
  async getDashboardStats(user) {
    ...
    if (user.role === ROLES.GENERAL_USER) {
      ...
      requestFilter = 'WHERE pr.department_id = ?';
    ...
    const [pendingMine] = await query(
      `SELECT COUNT(*) AS count FROM proc_requests pr
       WHERE pr.status = 'PENDING_DEPT_APPROVAL' AND pr.department_id = ?`,
      [user.department_id]
    );
