...
class ExportController {
  ...
  async generateDispatchPDF(req, res) {
    try {
      ...
      if (request.is_activity_request) {
        const fmtD = (d) => d ? new Date(d).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
        drawField('Activity Request', 'YES', col1, y);
        y += 34;
        drawField('Activity Start Date', fmtD(request.activity_start_date), col1, y);
        drawField('Activity End Date',   fmtD(request.activity_end_date),   col2, y);
        y += 34;
      }
  ...
  async generateReconciliationPDF(req, res) {
    try {
      ...
      if (request.is_activity_request) {
        ...
        drawField('Activity Request', 'YES', col1, y);
        y += 34;
        drawField('Activity Start Date', fmtDate(request.activity_start_date), col1, y);
        drawField('Activity End Date',   fmtDate(request.activity_end_date),   col2, y);
        y += 34;
      }
