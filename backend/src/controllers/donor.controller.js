...
exports.getDonorById = async (req, res) => {
  try {
    ...
    // Get budget lines — scope to user's department for non-Finance HOPs/Leads
    const needsDeptScope = needsDonorDeptFilter(req.user);
    const blWhere = needsDeptScope
      ? 'bl.donor_id = ? AND (bl.department_id = ? OR bl.department_id IS NULL)'
      : 'bl.donor_id = ?';
    const blParams = needsDeptScope
      ? [parseInt(id), req.user.department_id]
      : [parseInt(id)];
