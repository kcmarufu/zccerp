...
const RequestForm: React.FC = () => {
    ...
    const loadRequest = async () => {
      try {
        ...
        reset({
          ...
          currency: 'USD',
          isAdminRequest: false,
          isActivityRequest: Boolean(request.is_activity_request),
          activityStartDate: request.activity_start_date
            ? String(request.activity_start_date).slice(0, 10)
            : '',
          activityEndDate: request.activity_end_date
            ? String(request.activity_end_date).slice(0, 10)
            : '',
  ...
  const buildPayload = (data: RequestFormData) => ({
    ...
    is_activity_request: data.isActivityRequest ? 1 : 0,
    activity_start_date: data.isActivityRequest ? (data.activityStartDate || null) : null,
    activity_end_date:   data.isActivityRequest ? (data.activityEndDate   || null) : null,
  ...
  const handleSaveAndSubmit = async (data: RequestFormData) => {
    ...
    if (data.isActivityRequest) {
      ...
      if (data.activityEndDate < data.activityStartDate) { toast.error('Activity End Date cannot be before the Start Date'); return; }
    } else {
      toast.info('Note: This request is not marked as an Activity Request.');
    }
    try {
      ...
      <form>
        <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
          ...
          <Grid container spacing={3}>            {/* Partner Selection — locked when Admin request is active */}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <FormControlLabel
                    ...
                    label={
                      <Box display="flex" alignItems="center" gap={1}>
                        <ActivityIcon color={field.value ? 'warning' : 'disabled'} fontSize="small" />
                        <Box>
                          <Typography variant="body1">Activity Request</Typography>
                          <Typography variant="caption" color="text.secondary">
                            Tick if this float is for a scheduled activity
                          </Typography>
                        </Box>
                      </Box>
                    }
                    sx={{
                      ...
                      borderRadius: 1, p: 1, m: 0,
                      backgroundColor: field.value ? 'warning.50' : 'transparent'
                    }}
                  />
                )}
              />
            ...
            {/* Activity date fields — only shown when Activity Request is ticked */}
            {watchedIsActivityRequest && (
              <>
                <Grid item xs={12} md={6}>
                  <Controller
                    name="activityStartDate"
                    control={control}
                    rules={{ required: watchedIsActivityRequest ? 'Activity Start Date is required' : false }}
                    render={({ field }) => (
                      <TextField
                        {...field}
                        ...
                        helperText={errors.activityStartDate?.message || 'Enter the date the activity begins'}
                      />
                    )}
                  />
                </Grid>
                <Grid item xs={12} md={6}>
                  <Controller
                    ...
                    control={control}
                    rules={{ required: watchedIsActivityRequest ? 'Activity End Date is required' : false }}
                    render={({ field }) => (
                      <TextField
                        {...field}
                        ...
                        helperText={errors.activityEndDate?.message || 'Reconciliation is due 4 working days after this date'}
                      />
                    )}
                  />
                </Grid>
                <Grid item xs={12}>
                  <Alert severity="warning" icon={<ActivityIcon />}>
                    <strong>Activity Request:</strong> Reconciliation must be submitted within{' '}
                    <strong>4 working days</strong> of the Activity End Date. Submissions after this deadline will be marked as <strong>Late</strong>.
                  </Alert>
                </Grid>
              </>
            )}
