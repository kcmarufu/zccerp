...
<<<<<<< HEAD
  ...
  const navSections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          // Vendor management: Procurement Officer, Admin, and HOP/Lead Admin-HR
  ...
  const filteredSections = navSections
    .filter(section => {
      ...
      if (hasRole('PROCUREMENT_OFFICER') && !hasRole('ADMIN', 'FINANCE_CLERK', 'HEAD_OF_PROGRAMS', 'PROGRAM_LEAD') && section.id !== 'procurement') return false;
...
<<<<<<< HEAD
          ...
          {/* Spacer for xs when breadcrumbs are hidden */}
          ...
          <Popper open={notifOpen} anchorEl={notifAnchorRef.current} placement="bottom-end" transition disablePortal style={{ zIndex: 1300 }}>
            {({ TransitionProps }) => (
              <Grow {...TransitionProps} style={{ transformOrigin: 'right top' }}>
                <Paper elevation={8} sx={{ width: { xs: '92vw', sm: 360 }, maxHeight: 480, overflow: 'hidden', border: `1px solid ${theme.palette.divider}`, borderRadius: 2, mt: 0.5 }}>
