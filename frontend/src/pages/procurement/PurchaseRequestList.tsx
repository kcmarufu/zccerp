...
const PurchaseRequestList: React.FC = () => {
  ...
  // Client-side date + dept + donor filter, always newest first
  ...
  const [poDownloading, setPoDownloading] = useState<number | null>(null);

  const downloadPO = async (e: React.MouseEvent, reqId: number, reqCode: string) => {
    e.stopPropagation();
    setPoDownloading(reqId);
    try {
              ...
              requests.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((req) => (
                ...
                >
                  ...
                  <TableCell align="center">
                    ...
                    {(['PENDING_FINAL_FINANCE', 'COMPLETED'] as string[]).includes(req.status) && (
                      <Tooltip title="Download Purchase Order (PDF)">
                        <IconButton
                          size="small"
                          color="success"
                          disabled={poDownloading === req.id}
                          onClick={e => downloadPO(e, req.id, req.request_code)}
                        >
                          {poDownloading === req.id
