...
const VendorDatabase: React.FC = () => {
  ...
  // HOP, LEAD, Procurement Officer, and Admin can manage vendors
  const canManage = hasPermission('manage_vendors') || hasRole('HEAD_OF_PROGRAMS', 'PROGRAM_LEAD', 'PROCUREMENT_OFFICER', 'ADMIN');
  ...
  const handleExportExcel = () => {
    if (!vendors.length) { toast.warning('No vendors to export'); return; }
    const headers = ['#', 'Company Name', 'Category', 'Contact Person', 'Email', 'Phone', 'TIN Number', 'Reg Number', 'Prequalified', 'Active'];
    const rows = vendors.map((v: ProcVendor, i: number) => [
      i + 1, v.company_name || '', v.category || '', v.contact_person || '',
      v.email || '', v.phone || '', v.tin_number || '', v.registration_number || '',
      v.is_prequalified ? 'Yes' : 'No', v.is_active ? 'Yes' : 'No'
    ]);
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet([headers, ...rows]);
    ws['!cols'] = [4, 28, 18, 22, 26, 16, 16, 16, 12, 10].map(w => ({ wch: w }));
    XLSX.utils.book_append_sheet(wb, ws, 'Vendor Database');
    XLSX.writeFile(wb, `vendor-database-${new Date().toISOString().slice(0, 10)}.xlsx`);
    toast.success('Excel exported');
  };

  const handleExportPDF = () => {
    if (!vendors.length) { toast.warning('No vendors to export'); return; }
    const tableRows = vendors.map((v: ProcVendor, i: number) => `
      <tr>
        <td>${i + 1}</td>
        ...
        <td>${v.email || '—'}</td>
        <td>${v.phone || '—'}</td>
        <td style="text-align:center">${v.is_prequalified ? '<span style="color:#2e7d32;font-weight:bold">✓</span>' : '—'}</td>
        <td style="text-align:center">${v.is_active ? '<span style="color:#1565c0;font-weight:bold">✓</span>' : '<span style="color:#c62828">✗</span>'}</td>
      </tr>`).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Vendor Database</title>
<style>
  *{box-sizing:border-box;} body{font-family:Arial,sans-serif;font-size:11px;color:#1a1a1a;margin:0;padding:20px;}
  .header{border-bottom:2px solid #006064;padding:10px 0 12px;margin-bottom:16px;display:flex;justify-content:space-between;}
  .header h1{font-size:18px;margin:0;color:#006064;} .header p{margin:2px 0;font-size:11px;color:#444;}
  .org{font-size:11px;font-weight:bold;color:#006064;letter-spacing:.4px;margin-bottom:4px;}
